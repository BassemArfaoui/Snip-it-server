...
export class SolutionsService {
    ...
    async delete(solutionId: number, user: User): Promise<void> {
        ...
        this.profileService
            .calculateAndPersistScore(user.id)
            .catch(() => {
            });
    }
    ...
    async acceptSolution(
        ...
        const solution = await this.dataSource.transaction(async (manager) => {
            ...
            if (solution.issue.user.id !== user.id) {
                throw new ForbiddenException(
                    'Only the issue owner can accept a solution',
                );
            }
            ...
            await this.solutionRepo.markAsAccepted(
                solutionId,
                manager,
            );
        ...
        this.profileService
            .calculateAndPersistScore(solution.contributor.id)
            .catch(() => {
            });
